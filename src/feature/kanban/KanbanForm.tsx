...
import { DatePopver, EditDialog } from '@/feature/kanban'
import type { KANBAN_MODE, KANBAN_STATUS, KANBAN_VRIANT } from '@/types/dialog'
import { useEffect, useMemo, useState } from 'react'

const statusToVariant = (s: KANBAN_STATUS | ''): KANBAN_VRIANT => {
  switch (s) {
    case 'todo':
      return 'markAsTodo'
    case 'hold':
...
export default function KanbanForm(props: { type: KANBAN_MODE; onCancel: () => void }) {
  const { type, onCancel } = props

  const [mode, setMode] = useState<KANBAN_MODE>(type)
  useEffect(() => {
    setMode(type)
  }, [type])
  ...
  // 폼 필드 상태
  const [taskStatus, setTaskStatus] = useState<KANBAN_STATUS | ''>('')

  // 수정
  const [editOpen, setEditOpen] = useState(false)
  const [editVariant, setEditVariant] = useState<KANBAN_VRIANT>('markAsComplete')

  const editDialogTitle = useMemo(() => DIALOG_TYPES[editVariant].title, [editVariant])
  const editDialogDesc = useMemo(() => DIALOG_TYPES[editVariant].description, [editVariant])
      ...
      <fieldset className="mb-5" disabled={isRead}>
        ...
        <div className="flex gap-3">
          ...
          <SelectField
            ...
            value={taskStatus}
            onValueChange={(v) => setTaskStatus(v as KANBAN_STATUS)}
            disabled={isRead}
          />
